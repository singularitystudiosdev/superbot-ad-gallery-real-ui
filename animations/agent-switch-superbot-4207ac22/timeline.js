...
const CARDS = {
  ...
  context: { dur: 2.7, parts: ['Context', 'carries', 'from', { g: 'agent' }, { g: 'to' }, { g: 'agent' }] },
  lovable: {
...
function buildEnd(sec) {
  // the closing card: the mascot beside the line (this spot has no superbot.gg end screen)
  sec.innerHTML = '<div class="lock as-end"><div class="face"></div><div class="words"><div class="end-slide"><h1>With the agent to<br><span class="as-end-g">manage your agents</span></h1></div></div></div>';
...
const sceneIds = [...new Set(SEQUENCE.filter(([k]) => k === 'scene').map(([, id]) => id))];
const MODS = {};
...
let acc = 0;
const SECS = {};
for (const [kind, id, win] of SEQUENCE) {
  const full = kind === 'scene' ? Math.max(0.5, +MODS[id].dur || FALLBACK_DUR[id] || 8) : 0;
  const w = kind === 'scene' ? (win !== undefined && MODS[id].windows ? MODS[id].windows[win] : [0, full]) : null;
  const dur = kind === 'card' ? CARDS[id].dur : kind === 'end' ? END_DUR : w[1] - w[0];
  const key = kind === 'card' ? `c-${id}` : `s-${id}`;
  let sec = kind === 'scene' ? SECS[key] : null;
  const shared = !!sec;
  if (!sec) {
  ...
  SEGS.push({ kind, id, win, from: w ? w[0] : 0, shared, t0: +acc.toFixed(4), t1: +(acc + dur).toFixed(4), dur, sec });
  acc += dur;
}
...
for (const s of SEGS) {
  if (s.kind === 'card') s.card = buildCard(s.sec, CARDS[s.id]);
    ...
    s.mod = MODS[s.id];
    if (s.shared) { s.broken = SEGS.find((o) => o.sec === s.sec).broken; continue; } // mounted by its first window
...
function render(t) {
  ...
  if (active !== cur) {
    if (active && active.sec !== cur.sec) { active.sec.classList.remove('on'); active.sec.style.opacity = '0'; }
  ...
  if (cur.kind === 'card') {
    ...
    if (!cur.broken) {
      try { cur.mod.render(cur.from + lt, ctx); } catch (err) { report(cur, 'render', err); }
    }
  }
