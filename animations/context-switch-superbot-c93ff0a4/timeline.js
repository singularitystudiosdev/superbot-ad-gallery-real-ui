// context-switch-superbot: "Context carries from agent to agent." The engine (forked from do-that-too-superbot). The whole spot is a pure function of t, like
...
const SEQUENCE = [
  ['card', 'agents'], ['scene', 'tabs'], ['scene', 'claude'],
  ['card', 'carries'], ['scene', 'rules'],
  ['card', 'manage'], ['scene', 'refuse'],
  ['card', 'refuse'],
  ['end', 'end'],
];
// the durations a scene gets if its module fails to load (so the spot keeps its shape)
const FALLBACK_DUR = { tabs: 9.4, claude: 14.6, rules: 12.5, refuse: 16 };
...
const CARDS = {
  ...
  carries: { dur: 2.8, parts: ['Context', 'carries', 'from', { g: 'agent' }, { g: 'to' }, { g: 'agent' }] },
  // the mascot leads the line (mark: a live sb-mark beside the words instead of a trailing logo)
  manage: { dur: 3.2, parts: ['With', 'the', 'agent', 'to', { g: 'manage' }, { g: 'your' }, { g: 'agents' }], mark: 200 },
  refuse: { dur: 2.8, parts: ['And', 'when', 'your', 'other', 'agents', { html: '<span class="purple">refuse…</span>' }] },
};
...
await Promise.all(sceneIds.map(async (id) => {
  ...
  try {
    ...
    console.error(`[context-switch] scene "${id}" failed to load:`, err && err.stack ? err.stack : err);
...
function report(s, phase, err) {
  ...
  errSeen.add(key);
  console.error(`[context-switch] scene "${s.id}" threw in ${phase}:`, err && err.stack ? err.stack : err);
}
