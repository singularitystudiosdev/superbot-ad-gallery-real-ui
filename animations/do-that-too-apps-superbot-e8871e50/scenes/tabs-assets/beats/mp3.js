...
// Saved pills), "+90 more" and the Download all footer open, and the camera pushes onto the card.
...
export default {
  times(r) {
    ...
    T.chip = r + 0.35;
    T.card = T.chip + 0.3;
    T.row0 = T.card + 0.22; T.gap = 0.13;
    T.more = T.row0 + 9 * T.gap + 0.2;
    T.done = T.more + 0.22;
    T.foot = T.done + 0.1;
    T.end = T.foot + 2.1;
    return T;
  ...
  build(k, x) {
    ...
    const wrap = x.el(`<div class="yt-clip"><div class="sbx-group yt-card">
      ...
      <div class="yt-foot"><span class="yt-dl">${DL}<b>Download all (.zip)</b><em>${TOTAL}</em></span><span class="yt-saved">${FOLDER}Saved to <code>~/Music/superbot</code></span></div>
    </div></div>`);
    ...
    return {
      ...
      cams: [[T.card - 0.05, 1.2, () => x.F.el(card, 1.45, 0.88)]],
      render(t) {
