// The seven-request chat. Each ask is typed into the composer and sent, superbot routes it (its routing chip and the
// composer's platform chip follow the app), and the routed app answers with its own beat (./beats/*.js). An ask
// with no text is superbot continuing on its own (the clips go straight on to Gemini). The thread is
...
const APPS = {
  ...
  youtube: { name: 'YouTube', logo: brand('youtube-icon.svg'), sub: 'in superbot' },
...
const ASKS = [
  ...
  { app: 'doordash', mod: doordash, verb: 'Connecting to', ask: 'Let’s goo! Order me a burger to celebrate.' },
];
