/* ad.js - config for the "Schwab + Robinhood, ChatGPT vs Superbot" spot (variant A).
   Kit contract: animations/just-works-kit-42aac446 (engine.js + kit.css).
   Beats: ChatGPT cannot sign in to Schwab or Robinhood or pull live positions, so it cannot build the
   overview -> the "superbot can do it!" popup is clicked -> superbot signs in to both brokers read-only,
   pulls all 12 positions, lays the combined portfolio against 60/30/10 and maps a 7% trailing-stop line
   under every stock. No orders are placed. This folder is a copy of the superbot-only variant
   (just-works-portfolio-overview-superbot-42aac446): every asset, the site and the config below the gpt
   block are that spot's, unchanged, so the two variants share one page and one hub. */

export default {
  id: 'just-works-portfolio-overview-gpt-superbot-42aac446',
  title: 'Schwab + Robinhood, ChatGPT vs Superbot',
  // slug stays B's: it is the site's CSS scope class (.site-portfolio-overview), shared by both variants
  ...
  // the ChatGPT beat: it tries the Schwab positions page, that load fails, and the refusal explains why the
  // whole ask is beyond it. No cfg.dur.gpt on purpose: the beat lays itself out so the popup's click fits.
  gpt: {
    attempt: 'Opening client.schwab.com/app/accounts/positions',
    reply:
      "I can't sign in to your Schwab or Robinhood accounts or pull live positions from them, so I can't put your real holdings side by side or measure them against 60/30/10.\n\nWhat I can do instead: explain how a trailing stop works and how to work out your allocation by hand.",
  },
  card: null,
