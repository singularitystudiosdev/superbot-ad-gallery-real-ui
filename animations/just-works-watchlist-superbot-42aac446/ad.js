/* ad.js - config for the "Letterboxd + IMDb + Trakt, one watchlist" spot.
   ...
   Read-only by design: Superbot reads the Letterboxd, IMDb and Trakt watchlists, folds them into one list of
   distinct titles, looks each title up on the five services the viewer pays for, and adds, rates, removes,
   cancels and changes nothing. No ChatGPT beat and no card beat in this spot (variant B: the hub's own work is
   the whole point). Every figure below is read from data.js, so the hub, the steps and the page agree. */
...
export default {
  id: 'just-works-watchlist-superbot-42aac446',
  title: 'Letterboxd + IMDb + Trakt, one watchlist',
  slug: 'watchlist',
