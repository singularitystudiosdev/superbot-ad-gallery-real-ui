// new-number-one-superbot: "A new number one every week. Don't switch." The engine (forked from context-switch-superbot).
// scene 'slam' flashes the week's new #1 four times (GPT-6 / Claude Opus 5.5 / Gemini 3 / Grok 4.7), each faster than
// the last; scene 'arrive' is the real superbot hub, where a new agent joins the rail with a NEW dot and one click keeps
// the same thread going. The whole spot is a pure function of t, like
...
const FALLBACK_DUR = { slam: 7.0, arrive: 8.2 };
...
const CARDS = {
  // the switching bill: a new app, a new login and new habits, every time the #1 changes
  weekly: { dur: 3.6, parts: ['New', 'app.', 'New', 'login.', 'New', 'habits.', { g: 'Every' }, { g: 'week.' }] },
  // the payoff line, right after the hub keeps the thread: don't switch
  noswitch: { dur: 2.2, parts: ['Don’t', { g: 'switch.' }] },
};
...
function buildEnd(sec) {
  sec.innerHTML = '<div class="lock"><div class="face"></div><div class="words"><div class="end-slide"><h1>All the models.</h1><p>One superbot.</p></div></div></div>';
...
await Promise.all(sceneIds.map(async (id) => {
  ...
  try {
    ...
    console.error(`[new-number-one] scene "${id}" failed to load:`, err && err.stack ? err.stack : err);
...
function report(s, phase, err) {
  ...
  errSeen.add(key);
  console.error(`[new-number-one] scene "${s.id}" threw in ${phase}:`, err && err.stack ? err.stack : err);
}
