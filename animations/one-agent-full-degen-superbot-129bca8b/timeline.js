...
// one-agent-full-degen: the hub cropped to its thread, opening on the empty state, then the seven-request chat
// (scenes/tabs-assets/chat.js: superbot answers every ask itself, no switching) and the end card
const SEQUENCE = [
  ['scene', 'tabs'],
  ['end', 'end'],
];
...
// so the shared card timing factor K below keeps its value.
const CARDS = {
