// same-chat-new-model: "New model. Same chat." A new model lands, so the old way makes you start over: a new app,
// a sign in, and the same brief typed out again. In superbot the new model joins the thread you are already in.
// The engine (forked from context-switch-superbot, itself forked from do-that-too-superbot). The spot is a pure function of t, like
...
// rendered only while active; mount's return value is handed back as ctx.state) and the waffles-website end card.
...
// 8.0 montage + 2.4 card + 10.2 switch + 2.4 card + 4.0 end = 27.0 s
const SEQUENCE = [
  ['scene', 'montage'],   // the old way: new model, new app, new sign in, the same brief typed again
  ['card', 'over'],
  ['scene', 'switch'],    // the new model drops into the thread you are already in
  ['card', 'same'],
  ['end', 'end'],
];
// the durations a scene gets if its module fails to load (so the spot keeps its shape)
const FALLBACK_DUR = { montage: 8.0, switch: 10.2 };
const SCENE_FADE = 0.3;
const END_DUR = 4.0, DIP = 0.35;
...
const CARDS = {
  // the bill for a model swap: every app you move to starts empty
  over: { dur: 2.4, parts: ['Switching', 'models', 'means', { html: '<span class="dim">starting over.</span>' }] },
  // the answer: the new model keeps the thread
  same: { dur: 2.4, parts: ['New', { g: 'model' }, 'Same', { g: 'chat' }] },
};
...
// 'tabs' contributes no segment: its stylesheet is what @imports the hub window's own hub.css, and hubkit's
// markup (window frame, rail, composer) needs those rules even though the tabs intro never plays here.
const CSS_IDS = [...new Set([...sceneIds, 'tabs'])];
const MODS = {};
CSS_IDS.forEach((id) => {
...
await Promise.all(sceneIds.map(async (id) => {
  try {
    ...
    console.error(`[same-chat] scene "${id}" failed to load:`, err && err.stack ? err.stack : err);
...
function report(s, phase, err) {
  ...
  errSeen.add(key);
  console.error(`[same-chat] scene "${s.id}" threw in ${phase}:`, err && err.stack ? err.stack : err);
}
