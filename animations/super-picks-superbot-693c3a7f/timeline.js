// super-picks-superbot: "superbot picks the best agent for every task." The engine, forked from
// ../context-switch-superbot-c93ff0a4. The whole spot is a pure function of t, like
...
// ask the question, then three tasks routed by superbot, then the answer and the end card
const SEQUENCE = [
  ['card', 'best'], ['scene', 'picks'],
  ['card', 'never'],
  ['end', 'end'],
];
// the durations a scene gets if its module fails to load (so the spot keeps its shape)
const FALLBACK_DUR = { picks: 16 };
const SCENE_FADE = 0.3;
const END_DUR = 4.4, DIP = 0.35;

// ---------- text cards ----------
// A part is a word string, { g } for a word in the animated brand gradient, or { html } for styled words.
const CARDS = {
  best: { dur: 3.0, parts: ['Which', 'model', 'is', { g: 'best' }, 'this', 'week?'] },
  never: { dur: 2.9, parts: ['Never', 'pick', 'a', 'model', { g: 'again.' }] },
};
...
await Promise.all(sceneIds.map(async (id) => {
  ...
  try {
    ...
    console.error(`[super-picks] scene "${id}" failed to load:`, err && err.stack ? err.stack : err);
...
function report(s, phase, err) {
  ...
  errSeen.add(key);
  console.error(`[super-picks] scene "${s.id}" threw in ${phase}:`, err && err.stack ? err.stack : err);
}
