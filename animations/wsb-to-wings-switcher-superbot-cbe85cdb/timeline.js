...
// it-does-what-you-ask: the hub cropped to its thread, opening on the empty state, then the seven-request chat
// (scenes/tabs-assets/chat.js: Reddit, TradingView, Polymarket, Robinhood, Google Sheets, Robinhood, Reddit,
// DoorDash) and the end card
const SEQUENCE = [
  ['scene', 'tabs'],
  ['end', 'end'],
];
// the durations a scene gets if its module fails to load (so the spot keeps its shape)
const FALLBACK_DUR = { tabs: 61 };
